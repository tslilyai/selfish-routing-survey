...
$$\forall i,~\sum_{p\in P_i} f_p = r_i$$
   An \emph{optimal} (feasible) solution given such an input is the feasible flow assignment $f$ that minimizes the total cost $C(f)$, where
...
In the setting with centralized control, there is one clear optimal solution, which has been shown
solvable with a (non-integral) linear program (given a linear latency cost function).

Similar to how approximation algorithms work in a limited computational power model and how 
online algorithms work under a model of incomplete information, 
traffic routing algorithms typically assume a model in which there is a \emph{lack} of coordination
