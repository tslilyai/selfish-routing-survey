...
users.
We can formalize this problem as follows, as done by Roughgarden and Tardos in their seminal work on selfish routing~\cite{tardos}\cite{roughgarden}. We are given the following components:
\begin{itemize}
    \item A network $G = (V, E)$ comprised of $|V|$ nodes (e.g., locations or servers) and $|E|$ edges between each node acting as links
    \item A set of $k$ source-destination $\{(s_1,t_1), \cdots (s_k,t_k)\}$ representing traffic demands
    \item A rate $r_e$ for each $e\in E$ representing how fast traffic can flow through each edge 
\end{itemize}
