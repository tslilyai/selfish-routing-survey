...
because the altruistic model still achieves Nash equilibrium for any $\psi$ and cost function $c^\beta_p$: given any $\psi$, $c^\beta_p$ are continuous in the choice of path $p$, and we can apply the result of Theorem 1 of Mas-Colell~\cite{mascolell} \XXX{Vibhaa: can we mention what this theorem is in words, because no one will want to look it up}. Nash equilibrium is achieved at the flow solutions to the program
...
\begin{proof-sketch}
    ...
    \XXX{Vibhaa: What does this expression intuitively mean? Why is routing everything on the numerator? Is that the social cost? A little confused}
    ...
    \XXX{Vibhaa: What exactly are we doing in ``unfolding the definition of $\rho(T_\beta)$. Are we bounding the social welfare cost in terms of $\rho(T_\beta)$. If so what does $xl_e(x)$ represent in that context
    and why do we use $x = f_e^*$ and r = $\hat{f_e}$. Should I be thinking of this as routing the optimal as a whole and routing the nash equlibrium on one path and the rest on soemthing else?}
\end{proof-sketch}
...
\begin{theorem}
    For any $G$, demand rates $r$, $\ell \in L_d$, \XXX{Vibhaa: define $L_d$ up here for completeness}
...
is bounded above by the worst-case $\rho$ for any cost function $\ell \in L_d$ in $T_\beta$,\XXX{Vibhaa: should jsut mention $\rho(T_\beta)$ here in my opinion},  a two-node, two-link network with cost functions $\ell_1(x) = \ell(x)$ and $\ell_2(x) = c^\beta(r)$.
Thus, we only need to consider how bad $\rho(T_\beta)$ can get given any $l\in L_d$.

The key observation is that $\exists\lambda \in [0,1]$ s.t. $c^1(r\lambda) = c^\beta(r)$ \XXX{Vibhaa: why is this? What's the intuition}, where $c^1$ is the cost function with uniform altruism value $\beta=1$. Our strategy is three-part: 
\begin{enumerate}
...
We start with step (1): A Nash equilibrium flow in $T_\beta$ routing $r\lambda$ units from the source to the destination will put $r\lambda$ flow on the first link with $\ell_1 = \ell(x)$, and the rest \XXX{Vibhaa: isn't the rest just $0$ if you put all $r\lambda$ on the first. I guess I'm confused about what the total is, what's $x$ in the context of the previous theorem and might help to reference it/relate directly to it} on the second link with $\ell_2 = c^\beta(x)$. The solution optimizing social welfare will put all flow on the first link with latency function $\ell_1 = \ell(x)$.
This gives us a a bound on the price of anarchy when $r\lambda$ units of flow are sent through $T_\beta$:
    ...
    We now can perform step (3). First, we need to make an appropriate choice of $\lambda$. Without loss of generality we can consider latency functions \XXX{Vibhaa: think this definition should come much earlier} $\ell(x) = ax^i$ for some $i \le d$ to represent all functions $\ell \in L_d$. For these latency functions $\ell(x) = ax^i$, we know that 
    $c^1(r\lambda) = c^\beta(r)$ when $\lambda = \Big(\frac{1+\beta i}{1+i}\Big)^{\frac{1}{i}}$ \XXX{Vibhaa: cite or show something as to how this result came about}.
...
\XXX{Vibhaa: think we have to define parallel link networks and non-negative support}
...
\end{theorem}
One interesting observation is that this theorem leads to the following corrollary: because the theorem applies for a distribution in which a rate of $\bar{\beta}$ of users are completely altruistic and 1-$\bar{\beta}$ users are completely selfish, this is equivalent to $\bar{\beta}$ of the population being under centralized (coordinated) control. \XXX{Vibhaa: can you expand more on this? Should I be thinking of only having the altruistic part of the population and apply theorem 3? Not quite sure I follow the intuition/connection to centralized models which could help clarify the below corollary too}
\begin{corollary}
Given any parallel link network $G$, demand rates $r$, altruism density function $\psi$ with average altruism $\bar{\beta}$ and non-negative support, and convex and non-decreasing cost functions $\ell_e$,
if $\bar{\beta}$ fraction of traffic is controlled by a central authority, then
   $\rho(G,r,\ell,\psi) \le \frac{1}{\bar{\beta}}$
\end{corollary}
This is exactly the result proven by Roughgarden in 2004~\cite{roughgarden04} \XXX{Vibhaa: What is this paper about and why did they have to prove this result/what's the connection}, and provides an interesting connection between models that include altruism and models of centralized/coordinated traffic control.
