...
if $l_e$ is nondecreasing and convex for all $e$,
...
\begin{theorem}
For any input network $G$, demand rates $r$, $l_e \in L_d$,
...
$\rho(G,R,{L},\psi) \le \max_{l\in{L_d}} \max_{x,r\ge 0} \frac{rl(r)}{xl(x) + (r-x)(c^\beta(r))}$, where the RHS corresponds to the worst-case PoA for any cost function $l \in L_d$ in a two-node, two-
link network with cost functions $l_1(x) = l(x)$ and $l_2(x) = c^\beta{r}$.
...
$$\max_{l\in{L_d}} \max_{x,r\ge 0} \frac{rl(r)}{xl(x) + (r-x)(c^\beta(r))} = \Big(\Big(\frac{1+\beta i}{1+i}\Big)^{1/i}\Big(\frac{1+\beta i}{1+i} + 1 + \beta i\Big)+ 1 + \beta i\Big)^{-1}$$
    \XXX{TODO need to put in step moving from 2-link to NE / OPT = PoA in 2-link network}
