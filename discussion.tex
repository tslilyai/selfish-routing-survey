...
\begin{center}
 ...
 \hline\hline
        Cooperative & $\min_f\Big(\sum_e f_e\ell_e(f_e)\Big)$ & $\rho(G,r,\ell) = 1$ (optimal by definition) \\
 \hline
...
\hline
    ...
    & where $\gamma$ is the risk-aversion coefficient  & $\bullet~PRA(G,r,\ell,v,\gamma) = 1 + \gamma\upkappa $ for a series-parallel recursive graph\\
\hline
     Diverse Interests & $\sum_i \big(\sum_e\int_0^{f_e} \ell_e(t) + \omega_i\cdot \sigma_e dt\big)$ 
    & $\bullet~C^{ht}(g)> C^{hm}(f) \iff $ series-parallel for single-commodity network\\
    & where $\sigma$ is the deviation function, and $\omega_i$ is agent $i$'s diversity parameter $i$ & $\bullet~C^{ht}(g)> C^{hm}(f) \iff $ block-matching for multi-commodity network with average-respecting demand\\
\hline
\end{tabular}
\end{center}
