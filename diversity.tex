...
The key observation is that since $f$ and $g$ route the same amount of flow from the unique source to the unique sink, there must be a path $P$ where $f$ sends no less flow along than $g$ does. Since the network is series-parallel, for every edge $e$ in $P$, $f_e \ge g_e$, because a path in a series-parallel network can be broken up into some parts in series and some parts in parallel, which eventually breaks down to a simple path . and hence for any $r\in [0, r_{\max}]$, $c_p^r(f)\ge c_p^r(g)$.
